...
For each fixture (`badcli`, `wrappercli`, `goodcli`, `helptrapcli`):
...
3. Diff its verdicts against the fixture's `EXPECTED.md`:
   ...
   - For helptrapcli: 7.6 is a fail (B/C) — an evaluator that passes 7.6 here is rubber-stamping "argparse ⇒ pass" without checking for suppression. Everything else must match goodcli's verdicts.
