...
| 7.6 | `--help`/`-h` at every command level prints usage, exits 0, and never executes the command's action | B | C | FAIL@B | *(the essay's Layer 1 — "progressive help discovery")* Frameworks (cobra/click/argparse/clap/commander) inherit recursive help, and the framework contract covers exit-0 + never-executes — pass on that contract **unless** suppression is found; check every subcommand registration (each `add_parser`, each cobra `Command`), not just the root. Suppression tells: cobra `DisableFlagParsing`/help-func overrides/greedy positional `Args`; click `add_help_option=False`; argparse `add_help=False`, a `nargs=argparse.REMAINDER` positional, or `parse_known_args` feeding a hand-rolled dispatch. Hand-rolled parsers: confirm each dispatch level checks `-h`/`--help` *before* executing the action — a parser that treats `--help` as a positional and proceeds is the catastrophic fail (`cli delete X --help` deleting). A pass is evidenced by the *absence* of suppression at the construction sites (cite them) — there is no help handler to find in framework code. See spec Appendix E. |
